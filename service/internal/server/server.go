...
func New(opts Options) *fiber.App {
    ...
    // Auth: verify via Core API using incoming cookies/headers
    getUserID := func(c *fiber.Ctx) (string, error) {
        ...
        // API returns uid (number). Accept also userId (string) if present.
    ...
    app.Get("/v1/notes", func(c *fiber.Ctx) error {
        ...
        q := `SELECT id, user_id, title, content, COALESCE(todos,'null'::jsonb) AS todos, status, created_at, updated_at FROM notes 
    ...
    app.Get("/v1/notes/:id", func(c *fiber.Ctx) error {
        ...
        id := c.Params("id")
        var n Note
        if err := opts.DB.Get(&n, `SELECT id, user_id, title, content, COALESCE(todos,'null'::jsonb) AS todos, status, created_at, updated_at FROM notes WHERE id=$1 AND user_id=$2`, id, uid); err != nil {
    ...
    app.Post("/v1/notes", func(c *fiber.Ctx) error {
        ...
        if !hasTitle && !hasContent && !hasTodos { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "empty"}) }
        var n Note
...
func setStatus(opts Options, c *fiber.Ctx, status string) error {
    ...
    if err != nil { return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"success": false}) }
    id := c.Params("id")
    var n Note
    if err := opts.DB.Get(&n, `UPDATE notes SET status=$1, updated_at=now() WHERE id=$2 AND user_id=$3
